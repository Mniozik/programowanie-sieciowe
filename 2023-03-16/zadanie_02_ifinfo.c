/*
 * Compilation:  gcc -Wall ./zadanie_02_ifinfo.c -o ./ifinfo
 * Usage:        ./ifinfo
 */
...
void ifsinfo(int sfd, struct ifconf ifc)
{
  ...
  while (ptr < ifc.ifc_buf + ifc.ifc_len)
  {
    ...
    printf("inet_addr:%s\t |", inet_ntoa((struct in_addr)addr_in->sin_addr));
    // --- 
    ioctl(sfd, SIOCGIFHWADDR, ifr); //zadanie adresow MAC
